...
Model.prototype.getPayments = function (obj, cb) {
    if (arguments.length == 1) {
        cb = arguments[0];
        obj = {};
    }
    ...
    async.series({
        ...
        getInvoicePayment: function (cb) {
            ...
            _t.api(o, function (err,res) {
                ...
                if(err) return cb(new UserError('Не удалось получить пары Счёт-платёж', {err:err, o:o}));


                ip = res;

                for(var i in res){
                    ip_ids.push(res[i].daily_payment_id);
                }
        ...
        getPayments: function (cb) {
            ...
            _t.api(o, function (err, res) {
                ...
                if(err) return cb(new UserError('Не удалось получить платёжи привязанные к счету', {err:err, o:o}));
        ...
        if (err) {
            ...
            //}
            cb(null, new UserOk('noToastr', {payments:payments}));
        }
    });
};
