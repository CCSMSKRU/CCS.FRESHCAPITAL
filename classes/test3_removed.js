...
//     console.log(r);
// });
Model.prototype.getTest = function (obj, cb) {
    if (arguments.length == 1) {
        cb = arguments[0];
        obj = {};
    }
    ...
    async.series({
        getMerch:function(cb){
            ...
            _t.api(o, function (err, res) {
                if (err) return cb(new MyError('Не удалось получить мерчей',{o : o, err : err}));
                merchants = res;
                cb(null);
            });
        },
        ...
        if (err) {
            ...
            //}
            cb(null, new UserOk('Ок', {merchants:merchants}));
        }
    });
};
