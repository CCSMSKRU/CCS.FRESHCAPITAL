...
module.exports = function(obj, cb) {
	...
	switch (obj.type) {
		...
		case 'gov_seven':
			...
			if (is_to_date) {
				...
				function runDate(nextDate) {
					...
					res_array.push({
						...
						is_working_day: checkDate(moment(nextDate, 'DD.MM.YYYY'))
					});
				...
				for (var i = 0; i < payments_count; i++) {
					...
					res_array.push({
						...
						is_working_day: checkDate(lastDate)
					});
					lastDate = lastDate.add(1, 'd');
				}
			}
			...
			break;
		case 'custom':
			if (!obj.custom_calendar) return cb(new MyError('В метод не передан api'));
			if (isNaN(+obj.financing_id)) return cb(new MyError('В метод не передан financing_id финансирования'));
			...
			checkDate = date => {
				...
				if (obj.custom_calendar[mth]) {
					result = obj.custom_calendar[mth].indexOf(dayOMth.toString()) < 0;
				} else if (nw[year][mth]) {
					result = nw[year][mth].indexOf(dayOMth) < 0;
				}
